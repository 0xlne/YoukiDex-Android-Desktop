...
class IconPackUtils(val context: Context) {
    ...
    private fun getDrawableForName(name: String): Drawable? {
        if (isIconPackLoaded) {
            ...
            if (!item.isNullOrEmpty()) {
                ...
                if (id != 0) {
                    return loadedIconPackResource?.let { androidx.core.content.res.ResourcesCompat.getDrawable(it, id, context.theme) }
                }
            }
        }
        return null
    ...
    private fun getIconPackResources(id: Int, mContext: Context): Drawable? {
        return ResourcesCompat.getDrawable(loadedIconPackResource!!, id, context.theme)
    }
