...
class SoundsPreferences : PreferenceFragmentCompat() {
    ...
    private fun setupChargingToggle() {
        ...
        // else in the app reads it anyway — it's effectively just an
        // app-specific preference, with no need at all to store it in the
        // system's general settings. Modern Android rejects these unknown keys
        // by throwing an IllegalArgumentException instead of the expected
        // SecurityException that's actually handled, causing a full app crash
        // instead of a silent failure or a Toast.
        // Fix: we let SwitchPreferenceCompat save its state automatically in
        // the app's default SharedPreferences (its normal behavior with no
        // extra code needed), and any future code that plays the charging
        // sound reads this value via
        // PreferenceManager.getDefaultSharedPreferences(context)
        //     .getBoolean("charging_sounds_enabled", true)
        // instead of the system settings. No setOnPreferenceChangeListener is needed here.
    }
    ...
    private fun buildChmodCommands(filePath: String): String {
        val single = "chmod 644 \"$filePath\""
        val soundsDir = filePath.substringBeforeLast('/', "").ifEmpty { return single }
        val filesDir  = soundsDir.substringBeforeLast('/', "").ifEmpty { return single }
        val dataDir   = filesDir.substringBeforeLast('/', "").ifEmpty { return single }
        return "chmod 711 \"$dataDir\"\nchmod 711 \"$filesDir\"\nchmod 711 \"$soundsDir\"\nchmod 644 \"$filePath\""
    }
